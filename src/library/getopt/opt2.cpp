...
void Opt2::EatArgv(const char *optspec, const char *long_alias) {
    ...
    for (; ind != Argc; ind++) {
        ...
        if (*s == '-') {
            ...
            Stroka lname(s, eq ? (size_t)(eq - s) : (size_t)strlen(s));
...
const char *Opt2::Arg(char opt, const char *help, const char *def, bool required) {
    ...
    return p.IsFound? p.ActualValue.empty()? 0 : p.ActualValue.back() : def;
}

// For options with parameters
const char *Opt2::Arg(char opt, const char *help, Stroka def, bool required) {
    Opt2Param &p = GetInternal(opt, 0, help, required);
    if (!p.HasArg)
        ythrow yexception() << "Opt2::Arg called for '" <<  opt << "' which is an option without argument";
    p.DefValueStr = def;
    p.DefValue = p.DefValueStr.begin();
    return p.IsFound? p.ActualValue.empty()? 0 : p.ActualValue.back() : p.DefValue;
}

// Options with parameters that can be specified several times
const yvector<const char*> &Opt2::MArg(char opt, const char *help) {
    Opt2Param &p = GetInternal(opt, 0, help, false);
    p.MultipleUse = true;
    if (!p.HasArg)
        ythrow yexception() << "Opt2::Arg called for '" <<  opt << "' which is an option without argument";
...
int Opt2::AutoUsage(const char *free_arg_names) {
    ...
    for (yvector<Opt2Param>::iterator i = Specs.begin(); i != Specs.end(); ++i) {
        const char *hlp = i->HelpUsage? i->HelpUsage : i->HasArg? "<arg>" : "";
        if (!i->HasArg || i->IsRequired)
            fprintf(where, "  -%c %s\n", i->opt, hlp);
        else if (!i->IsNumeric)
            fprintf(where, "  -%c %s [Default: %s]\n", i->opt, hlp, i->DefValue);
        else
            fprintf(where, "  -%c %s [Def.val: %li]\n", i->opt, hlp, (long)(uintptr_t)i->DefValue);
        if (i->LongOptName)
            fprintf(where, "    --%s%s - same as -%c\n", i->LongOptName, i->HasArg? "=<argument>" : "", i->opt);
    }
    if (OptionMissingArg)
    ...
    for (yvector<Stroka>::const_iterator errIt = UserErrorMessages.begin(); errIt != UserErrorMessages.end(); ++errIt)
        fprintf(where, " *** %s\n", ~(*errIt));
