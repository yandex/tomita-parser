...
struct Opt2Param {
    ...
    Opt2Param() :
        HasArg(false), IsFound(0), IsNumeric(0), IsRequired(0), MultipleUse(0),
        DefValue(0), HelpUsage(0), LongOptName(0) {}
};

struct IntRange {
    int Left, Right;
    //IntRange() {}
    IntRange(int both) : Left(both), Right(both) {}
    IntRange(int left, int right) : Left(left), Right(right) {}
};

class Opt2 {
public:
    Opt2() {}
    Opt2(int argc, char* const* argv, const char *optspec, IntRange free_args_num = -1, const char *long_alias = 0) {
    ...
    void Init(int argc, char* const* argv, const char *optspec, IntRange free_args_num = -1, const char *long_alias = 0);
    ...
    const char *Arg(char opt, const char *helpUsage) {
        return Arg(opt, helpUsage, 0, true);
    }
    const char *Arg(char opt, const char *helpUsage, Stroka defValue, bool required = false);
    ...
    void AddError(const char* message = NULL);
...
private:
    ...
    yvector<Stroka> UserErrorMessages;
