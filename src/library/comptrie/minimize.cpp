...
typedef yvector<size_t> TOffsetList;
typedef yhash<size_t, TOffsetList> TPieceIndex;

typedef TPair<size_t, size_t> TSizePair;
typedef yvector<TSizePair> TSizePairVector;
typedef yvector<TSizePairVector> TSizePairVectorVector;
...
public:
    ...
    void Add(size_t key, size_t value) {
        ...
        for (TSizePairVector::iterator it = sublist.begin(); it != sublist.end(); it++) {
            if (it->first == key) {
                it->second = value;
    ...
    size_t Get(size_t key) const {
        ...
        for (TSizePairVector::const_iterator it = sublist.begin(); it != sublist.end(); it++) {
            if (it->first == key)
                return it->second;
        }
