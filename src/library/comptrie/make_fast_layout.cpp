...
size_t TTrieMeasurer::MeasureSubtrie(size_t rootOffset, bool isNewPath) {
    YASSERT(rootOffset < Trie.Length);
...
typedef yvector<size_t> TLevelNodes;
...
private:
    ...
    yvector<TLevel> Trace;
    TOffsetIndex BackIndex;
    yvector<TNode> CentralWord;
...
private:
    ...
    void AddCascade(size_t step) {
        YASSERT(!(step & (step - 1))); // Should be a power of 2.
        while (step > 0) {
...
size_t RawCompactTrieFastLayoutImpl(TOutputStream& os, const TOpaqueTrie& trie, bool verbose) {
