    ...
    : TChunkedDataReader(blob)
{
    ...
    size_t index = 0;
    for (yvector<Stroka>::const_iterator it = Names.begin(); it != Names.end(); ++it, ++index) {
        if (!it->empty())
...
TNamedChunkedDataWriter::TNamedChunkedDataWriter(TOutputStream& slave)
    : TChunkedDataWriter(slave)
{
}

TNamedChunkedDataWriter::~TNamedChunkedDataWriter() throw () {
}
...
void TNamedChunkedDataWriter::NewBlock(const Stroka& name) {
    if (!name.empty()) {
