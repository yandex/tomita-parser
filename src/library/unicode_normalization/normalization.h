...
namespace NUnicode {
    ...
    namespace NPrivate {
        inline const wchar32* Decomposition(const TDecompositionTable& table, wchar32 ch) {
            return table.Get(ch, static_cast<const wchar32*>(NULL));
        }
        ...
        private:
            ...
            class TKey : public TPair<wchar32, wchar32> {
            public:
                inline TKey(wchar32 a, wchar32 b)
                    : TPair<wchar32, wchar32>(a, b)
                {}
            ...
            typedef yhash<TKey, wchar32, THash<TKey> > TData;
            TData Data;
        ...
        typedef TPair<wchar32, TCombining> TSymbol;
        typedef yvector<TSymbol> TBuffer;
        ...
        public:
            ...
            inline void DoComposition(TBuffer& buffer) {
                ...
                YASSERT(DecompositionCombining(lead) == 0);
                buffer[0] = TSymbol(lead, 0);
            }
        };

        template <ENormalization N>
        inline bool Normalized(const TChar* begin, const TChar* end) {
            TCombining lastCanonicalClass = 0;
            for (const TChar* i = begin; i != end;) {
    ...
    private:
        ...
        inline void AddChar(wchar32 c, T& out) {
            ...
            if (decompBegin) {
                while (*decompBegin) {
                    YASSERT(Decompositor.Decomposition(*decompBegin) == NULL);
        ...
        template <class T>
        inline void DoNormalize(const TChar* begin, const TChar* end, T& out) {
            Buffer.clear();
    ...
    public:
        ...
        template <class T>
        inline void Normalize(const TChar* begin, const TChar* end, T& out) {
            if (NPrivate::Normalized<Norm>(begin, end)) {
                for (const TChar* i = begin; i != end; ++i) {
                    WriteSymbol(*i, out);
                }
            } else {
        ...
        template <class T>
        inline void Normalize(const TChar* begin, size_t len, T& out) {
            return Normalize(begin, begin + len, out);
        }

        inline Wtroka Normalize(const Wtroka& src) {
...
inline Wtroka Normalize(const TChar* str, size_t len) {
    Wtroka res;
    res.reserve(len);
...
template <NUnicode::ENormalization N>
inline Wtroka Normalize(const Wtroka& str) {
