...
static bool IsEqualImpl(const Message& m1, const Message& m2, yvector<Stroka>* differentPath);

namespace NImpl {
    template <FieldDescriptor::CppType CppType, bool useDefault>
    struct TCompareValue {
        typedef typename TCppTypeTraits<CppType>::T T;
        static inline bool IsEqual(T value1, T value2, yvector<Stroka>*) {
    ...
    struct TCompareValue<FieldDescriptor::CPPTYPE_MESSAGE, useDefault> {
        static inline bool IsEqual(const Message* value1, const Message* value2, yvector<Stroka>* differentPath) {
    ...
    public:
        static inline bool IsEqual(const Message& m1, const Message& m2, const FieldDescriptor& field, yvector<Stroka>* differentPath) {
    ...
    private:
        static bool IsEqualSingle(const Message& m1, const Message& m2, const FieldDescriptor* field, yvector<Stroka>* differentPath) {
        ...
        static bool IsEqualRepeated(const Message& m1, const Message& m2, const FieldDescriptor* field, yvector<Stroka>* differentPath) {
        ...
        static inline bool IsEqualRepeatedValue(const Message& m1, const Message& m2, const FieldDescriptor* field, int index, yvector<Stroka>* differentPath) {
    ...
    template <bool useDefault>
    bool IsEqualField(const Message& m1, const Message& m2, const FieldDescriptor& field, yvector<Stroka>* differentPath) {
        #define CASE_CPPTYPE(cpptype) case FieldDescriptor::CPPTYPE_##cpptype: { \
            bool r = TCompareField<FieldDescriptor::CPPTYPE_##cpptype, useDefault>::IsEqual(m1, m2, field, differentPath); \
            if (!r && !!differentPath) { differentPath->push_back(field.name()); } \
            return r; \
        }
...
template <bool useDefault>
bool IsEqualImpl(const Message& m1, const Message& m2, yvector<Stroka>* differentPath) {
    const Descriptor* descr = m1.GetDescriptor();
    if (descr != m2.GetDescriptor()) {
        return false;
    }
    for (int i = 0; i < descr->field_count(); ++i)
        if (!NImpl::IsEqualField<useDefault>(m1, m2, *descr->field(i), differentPath)) {
            return false;
        }
    return true;
}
...
bool IsEqual(const Message& m1, const Message& m2) {
    return IsEqualImpl<false>(m1, m2, NULL);
}

bool IsEqual(const Message& m1, const Message& m2, Stroka* differentPath) {
    yvector<Stroka> differentPathVector;
    yvector<Stroka>* differentPathVectorPtr = !!differentPath ? &differentPathVector : 0;
    bool r = IsEqualImpl<false>(m1, m2, differentPathVectorPtr);
    if (!r && differentPath) {
        *differentPath = JoinStroku(differentPathVector.rbegin(), differentPathVector.rend(), "/");
    }
    return r;
}

bool IsEqualDefault(const Message& m1, const Message& m2) {
    return IsEqualImpl<true>(m1, m2, NULL);
}
