...
Wtroka RemoveWideTokenPrefix(TWideToken& token) {
    const size_t prefixLen = token.SubTokens[0].PrefixLen;
    Wtroka res(token.Token, prefixLen);
    token.Token += prefixLen;
    token.Leng -= prefixLen;
    token.SubTokens[0].PrefixLen = 0;
    for (size_t i = 0; i < token.SubTokens.size(); ++i) {
        token.SubTokens[i].Pos -= prefixLen;
    }
    return res;
}

Wtroka RemoveWideTokenSuffix(TWideToken& token) {
    const size_t suffixLen = token.SubTokens.back().SuffixLen;
    Wtroka res(token.Token + token.SubTokens.back().EndPos(), suffixLen);
...
bool CheckWideTokenSplit(const TWideToken& token, size_t pos) {
    YASSERT(pos < token.SubTokens.size() - 1);

    const TCharSpan& subtoken = token.SubTokens[pos];
    const TCharSpan& subtokenNext = token.SubTokens[pos + 1];

    return (subtoken.Type != subtokenNext.Type)
        || (subtoken.Type != TOKEN_WORD)
        || ((subtoken.TokenDelim != TOKDELIM_APOSTROPHE) && (subtoken.TokenDelim != TOKDELIM_MINUS));
}
...
bool CheckWideTokenDotSplit(const TWideToken& token, size_t pos) {
    ...
    if ((token1.Type == TOKEN_WORD || (token1.Type == TOKEN_NUMBER && pos == 0))
        && token2.Type == TOKEN_WORD
        && (::IsUpper(token.Token[token2.Pos]) || ::IsTitle(token.Token[token2.Pos]))) {
        return true;
    }
...
TWideToken ExtractWideTokenRange(const TWideToken& tok, size_t start, size_t end) {
    YASSERT(start < tok.SubTokens.size());
    YASSERT(end < tok.SubTokens.size());
