...
const char* const ST_TYPE_STRINGS[] = {
    "ST_NOBRK",
    "ST_SENTBRK",
    "",
    "ST_PARABRK",
...
const char* GetSpaceTypeString(TBreakType type) {
    type &= ~ST_ZONEOPN; // todo: implement
    type &= ~ST_ZONECLS;
    return ST_TYPE_STRINGS[type];
}
...
static NLP_TYPE GuessTypeByWordT(const TChr *w, size_t len)
{
    ...
    switch (state) {
        case G_START: return NLP_MARK;
        case G_INT: return NLP_INTEGER;
        case G_DOT: return NLP_FLOAT; // NLP_MARK?
        case G_FRAC: return NLP_FLOAT;
    }
    YASSERT(0);
    return NLP_MARK;
}
...
NLP_TYPE DetectNLPType(const TTokenStructure& subtokens) {
    YASSERT(subtokens.size() >= 1);
    for (size_t i = 1; i < subtokens.size(); ++i) {
        YASSERT(subtokens[i].Type == TOKEN_WORD || subtokens[i].Type == TOKEN_NUMBER);
