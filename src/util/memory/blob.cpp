...
public:
    ...
    static TNullBlobBase* CommonBase() throw ();
};

TNullBlobBase* TNullBlobBase::CommonBase() throw () {
...
class TDynamicBlobBase: public TBlob::TBase,
    ...
    typedef TRefCounted<TDynamicBlobBase, TCounter> TRefBase;
public:
    inline TDynamicBlobBase() {
    }

    virtual ~TDynamicBlobBase() throw () {
    }
...
template <class TCounter>
class TBufferBlobBase: public TBlob::TBase,
                       public TRefCounted<TBufferBlobBase<TCounter>, TCounter> {
    typedef TRefCounted<TBufferBlobBase, TCounter> TRefBase;
public:
    inline TBufferBlobBase(TBuffer& buf) {
        Buf_.Swap(buf);
    }

    virtual ~TBufferBlobBase() throw () {
    }
    ...
    inline const TBuffer& Buffer() const throw () {
        return Buf_;
    }
...
template <class TCounter>
class TStringBlobBase: public TBlob::TBase,
                       public TRefCounted<TStringBlobBase<TCounter>, TCounter> {
    typedef TRefCounted<TStringBlobBase, TCounter> TRefBase;
public:
    inline TStringBlobBase(const Stroka& s)
        : S_(s)
    {
    }

    virtual ~TStringBlobBase() throw () {
    }
    ...
    inline const Stroka& String() const throw () {
        return S_;
    }
...
template <class TCounter>
class TMappedBlobBase: public TBlob::TBase,
                       public TRefCounted<TMappedBlobBase<TCounter>, TCounter> {
    typedef TRefCounted<TMappedBlobBase<TCounter>, TCounter> TRefBase;
public:
...
TBlob::TBlob() throw ()
    : S_(0, 0, TNullBlobBase::CommonBase())
{
}
...
static inline TBlob CopyConstruct(const void* data, size_t len) {
    typedef TDynamicBlobBase<TCounter> Base;
    THolder<Base> base(new (len) Base);

    YASSERT(base->Length() == len);
...
static inline TBlob ConstructFromMap(const TMemoryMap& map, ui64 offset, size_t length) {
    typedef TMappedBlobBase<TCounter> TBase;
...
static inline TBlob ConstructAsMap(const T& t) {
    TMemoryMap::OMode mode = static_cast<TMemoryMap::OMode>(
        precharge ? (TMemoryMap::oRdOnly | TMemoryMap::oPrecharge) : TMemoryMap::oRdOnly);
...
TBlob TBlob::FromFileSingleThreaded(const Stroka& path) {
    return ConstructAsMap<TSimpleCounter, false>(path);
}

TBlob TBlob::FromFile(const Stroka& path) {
...
TBlob TBlob::PrechargedFromFileSingleThreaded(const Stroka& path) {
    return ConstructAsMap<TSimpleCounter, true>(path);
}

TBlob TBlob::PrechargedFromFile(const Stroka& path) {
...
static inline TBlob ReadFromFile(const TFile& file, ui64 offset, size_t length) {
    typedef TDynamicBlobBase<TCounter> TBase;
    THolder<TBase> base(new (length) TBase);

    YASSERT(base->Length() == length);
...
TBlob TBlob::FromFileContentSingleThreaded(const Stroka& path) {
    TFile file(path, RdOnly);
    return ConstructFromFileContent<TSimpleCounter>(file, 0, file.GetLength());
}

TBlob TBlob::FromFileContent(const Stroka& path) {
    TFile file(path, RdOnly);
...
static inline TBlob ConstructFromBuffer(TBuffer& in) {
    typedef TBufferBlobBase<TCounter> TBase;
...
template <class TCounter>
static inline TBlob ConstructFromString(const Stroka& s) {
    typedef TStringBlobBase<TCounter> TBase;
...
TBlob TBlob::FromStringSingleThreaded(const Stroka& s) {
    return ConstructFromString<TSimpleCounter>(s);
}

TBlob TBlob::FromString(const Stroka& s) {
