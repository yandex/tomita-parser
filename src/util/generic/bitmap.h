...
struct TBigToSmallDataCopier {
    ...
    STATIC_ASSERT(sizeof(TTargetChunk) < sizeof(TSourceChunk));
    STATIC_ASSERT(0 == sizeof(TSourceChunk) % sizeof(TTargetChunk));

    static const size_t BLOCK_SIZE = sizeof(TSourceChunk) / sizeof(TTargetChunk);
    ...
    static inline void CopyChunk(TTargetChunk* target, TSourceChunk source) {
        ...
        ::ReverseCopy(c.SmallData, c.SmallData + ARRAY_SIZE(c.SmallData), target);
#else
        ::Copy(c.SmallData, c.SmallData + ARRAY_SIZE(c.SmallData), target);
#endif
    }

    static inline void Copy(TTargetChunk* target, size_t targetSize, const TSourceChunk* source, size_t sourceSize) {
        YASSERT(targetSize >= sourceSize * BLOCK_SIZE);
...
struct TSmallToBigDataCopier {
    ...
    STATIC_ASSERT(sizeof(TTargetChunk) > sizeof(TSourceChunk));
    STATIC_ASSERT(0 == sizeof(TTargetChunk) % sizeof(TSourceChunk));

    static const size_t BLOCK_SIZE = sizeof(TTargetChunk) / sizeof(TSourceChunk);
    ...
    static inline void Copy(TTargetChunk* target, size_t targetSize, const TSourceChunk* source, size_t sourceSize) {
        YASSERT(targetSize * BLOCK_SIZE >= sourceSize);
...
template <typename TTargetChunk, typename TSourceChunk>
struct TDataCopier:
    public TSelectType<TSameType<TTargetChunk, TSourceChunk>::Result
        , TUniformDataCopier<TTargetChunk>
        , typename TSelectType<TIsSmaller<TTargetChunk, TSourceChunk>::Result
            , TBigToSmallDataCopier<TTargetChunk, TSourceChunk>
...
private:
    STATIC_ASSERT(TTypeTraits<TChunk>::IsUnsignedInt);
    ...
    static const TChunk FullChunk = ~TChunk(0);
    ...
    typedef typename TSelectType<sizeof(TChunk) < sizeof(unsigned int), unsigned int, TChunk>::TResult TIntType;
...
private:
    ...
    struct TSetOp {
        static inline TChunk Op(TChunk src, TChunk mask) {
            return src | mask;
        }
    };
    ...
    void UpdateRange(size_t start, size_t end) {
        ...
        size_t excludeFirst = 0;
        size_t includeLast = 1;

        if (startBitOffset || startChunk == endChunk) {
            TChunk updateMask = FullChunk << startBitOffset;
            if (startChunk == endChunk && endBitOffset) {
                updateMask &= ~(FullChunk << endBitOffset);
            }
            Mask.Data[startChunk] = TUpdateOp::Op(Mask.Data[startChunk], updateMask);
            excludeFirst = 1;
        }

        if (endBitOffset && endChunk < Mask.GetChunkCapacity() && endChunk > startChunk) {
            Mask.Data[endChunk] = TUpdateOp::Op(Mask.Data[endChunk], ~(FullChunk << endBitOffset));
            includeLast = 0;
        }

        for (size_t i = startChunk + excludeFirst; i < Min(endChunk + includeLast, Mask.GetChunkCapacity()); ++i) {
            Mask.Data[i] = TUpdateOp::Op(Mask.Data[i], FullChunk);
        }
    }
...
public:
    ...
    void Export(size_t pos, TTo& to) const {
        STATIC_ASSERT(TTypeTraits<TTo>::IsUnsignedInt);
        to = 0;
        ...
        if ((pos & ModMask) == 0) {
            ...
            const static size_t copyToSize = (sizeof(TChunk) >= sizeof(TTo)) ? (sizeof(TChunk) / sizeof(TTo)) + 2 : 3;
    ...
    FORCED_INLINE void Reserve(size_t bitCount) {
        VERIFY(Mask.ExpandBitSize(bitCount), "Exceeding bitmap storage capacity");
    }

    FORCED_INLINE size_t ValueBitCount() const {
        ...
        return nonZeroChunk || Mask.Data[nonZeroChunk]
            ? nonZeroChunk * BitsPerChunk + GetValueBitCountImpl(TIntType(Mask.Data[nonZeroChunk]))
            : 0;
    }
    ...
    template <class T>
    FORCED_INLINE bool HasAny(const TBitMapOps<T>& bitmap) const {
        return HasAny(TThis(bitmap));
    }

    FORCED_INLINE bool HasAny(const TChunk& val) const {
    ...
    template <class T>
    FORCED_INLINE bool HasAll(const TBitMapOps<T>& bitmap) const {
        return HasAll(TThis(bitmap));
    }

    FORCED_INLINE bool HasAll(const TChunk& val) const {
    ...
    template <class T>
    FORCED_INLINE TThis& SetDifference(const TBitMapOps<T>& bitmap) {
        return SetDifference(TThis(bitmap));
    }

    FORCED_INLINE TThis& SetDifference(const TChunk& val) {
        Mask.Data[0] &= ~val;
        return *this;
    }
    ...
    //     ...
    // }
    // See FOR_EACH_BIT macro definition at the bottom
...
#define FOR_EACH_BIT(var, bitmap) for (size_t var = (bitmap).FirstNonZeroBit(); var != (bitmap).Size(); var = (bitmap).NextNonZeroBit(var))
