#pragma once

#include "cstriter.h"

#include <util/generic/bitmap.h>

template <class TSet>
class TStrSpnImpl {
public:
    inline TStrSpnImpl(const char* b, const char* e) {
        Init(b, e);
    }

    inline TStrSpnImpl(const char* s) {
        Init(s, TCStringEndIterator());
    }

    //FirstOf
    template <class It>
    inline It FindFirstOf(It b, const char* e) const throw () {
        return FindFirst<false>(b, e);
    }

    template <class It>
    inline It FindFirstOf(It s) const throw () {
        return FindFirst<false>(s, TCStringEndIterator());
    }

    //FirstNotOf
    template <class It>
    inline It FindFirstNotOf(It b, const char* e) const throw () {
        return FindFirst<true>(b, e);
    }

    template <class It>
    inline It FindFirstNotOf(It s) const throw () {
        return FindFirst<true>(s, TCStringEndIterator());
    }

private:
    template <bool Result, class It1, class It2>
    inline It1 FindFirst(It1 b, It2 e) const throw () {
        while (b != e && (S_.Get((ui8)*b) == Result)) {
            ++b;
        }

        return b;
    }

    template <class It1, class It2>
    inline void Init(It1 b, It2 e) {
        while (b != e) {
            S_.Set((ui8)*b++);
        }
    }

private:
    TSet S_;
};

typedef TStrSpnImpl<TBitMap<256> > TCompactStrSpn;
