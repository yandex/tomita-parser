...
size_t Tr::FindFirstChangePosition(const Stroka& str) const {
    for (Stroka::const_iterator it = str.begin(); it != str.end(); ++it) {
...
void Tr::Do(Stroka& str) const {
    const size_t changePosition = FindFirstChangePosition(str);
    if (changePosition == Stroka::npos)
        return;
    for (Stroka::iterator it = str.begin() + changePosition; it != str.end(); ++it)
        *it = ConvertChar(*it);
}
