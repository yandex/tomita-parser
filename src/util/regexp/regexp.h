...
public:
    TRegExBase(const char *regExpr = NULL, int cflags = REG_EXTENDED);

    virtual ~TRegExBase();

    int Exec(const char *str, regmatch_t pmatch[], int eflags, int nmatches = NMATCHES) const;
    void Compile(const char *regExpr, int cflags = REG_EXTENDED);
    bool IsCompiled() const;
    int GetCompileOptions() const;
    Stroka GetRegExpr() const;
};

class TRegExMatch : public TRegExBase
{
public:
    TRegExMatch(const char *regExpr = NULL, int cflags = REG_NOSUB | REG_EXTENDED);
...
public:
    TRegExSubst(const char *regExpr = NULL, int cflags = REG_EXTENDED);

    Stroka Replace(const char *str, int eflags = 0);
    int ParseReplacement(const char *replacement);
    DECLARE_NOCOPY(TRegExSubst);
};
