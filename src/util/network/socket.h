...
template <class T>
static inline int SetSockOpt(SOCKET s, int level, int optname, T opt) throw () {
    return setsockopt(s, level, optname, (const char*)&opt, sizeof(opt));
}

template <class T>
static inline int GetSockOpt(SOCKET s, int level, int optname, T& opt) throw () {
...
public:
    ...
    public:
        ...
        friend inline bool operator== (const TIterator& l, const TIterator& r) throw () {
            return l.C_ == r.C_;
        }
        ...
        inline struct addrinfo& operator* () const throw () {
            return *C_;
        }
    ...
    TNetworkAddress(const Stroka& host, ui16 port);
    TNetworkAddress(const Stroka& host, ui16 port, int flags);
    ~TNetworkAddress() throw ();
...
private:
    struct addrinfo* Info() const throw ();
...
public:
    TSocketInput(const TSocket& s) throw ();
    virtual ~TSocketInput() throw ();

    const TSocket &GetSocket() const throw() {
        return S_;
    }
...
public:
    TSocketOutput(const TSocket& s) throw ();
    virtual ~TSocketOutput() throw ();
