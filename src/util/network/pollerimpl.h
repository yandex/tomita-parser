...
static inline int MicroToMilli(int timeout) throw () {
    if (timeout) {
        /*
...
public:
    ...
    inline void Remove(int fd) throw () {
        ...
        VERIFY(!(Kevent(Fd_, e, 2, 0, 0, 0) == -1 && errno != ENOENT), "kevent remove failed: %s", LastSystemErrorText());
    }
    ...
    inline size_t Wait(TEvent* events, size_t len, int timeout) throw () {
        ...
        VERIFY(ret >= 0, "kevent failed: %s", LastSystemErrorText());
...
public:
    ...
    inline size_t Wait(TEvent* events, size_t len, int timeout) throw () {
        ...
        VERIFY(ret >= 0, "epoll wait error: %s", LastSystemErrorText());
...
public:
    ...
    inline void SetImpl(void* data, SOCKET fd, int what) {
        {
            TGuard<TMyMutex> g(CommandLock_);
    ...
    inline size_t WaitBase(TEvent* events, size_t len, int timeout) throw () {
        ...
        {
            TGuard<TMyMutex> lock(CommandLock_);

            for (typename yvector<TCommand>::const_iterator command = Commands_.begin();
        ...
        VERIFY(ret >= 0 && (size_t)ret <= len, "select error: %s", LastSystemErrorText());
...
private:
    struct TCommand {
        ...
        TCommand(SOCKET fd, int filter, void* cookie)
            : Fd_(fd), Filter_(filter), Cookie_(cookie)
        {}
