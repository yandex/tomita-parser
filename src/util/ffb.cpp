...
inline int sf_blank(char **fb, char *buf, size_t fb_sz) {
    while (isspace((ui8)*buf)) buf++;
    if (!*buf) { *fb = 0; return 0; }
    fb_sz--;
    int n = 1;
    fb[0] = buf;
    while (*buf && *buf != 10 && n < (int)fb_sz) {
...
void TLineSplitter::operator() (char *p, yvector<char*>& fields) const {
    if (!p || !*p)
        return;
    char *q = p;
    while (1) {
...
void TSFReader::Open(const char *fname, i32 nfrq, size_t vbuf_size) {
    ...
    if (!isatty(fileno(File)))
        setvbuf(File, 0, _IOFBF, vbuf_size);
}
...
bool TSFReader::NextLine(segmented_string_pool* pool) {
    ...
    if (FieldsRequired != -1 && FieldsRequired != (int)NF)
        throw yexception() << File.name() << " line " << NR << ": " << NF << " fields, expected " << FieldsRequired;
...
int prnstr::f(const char *c, ...) {
    ...
    while ((k=vsnprintf(buf+pos, n, c, params)) >= n) {
        n+=asize, asize*=2; while (k+pos >= n) n+=asize, asize*=2;
...
FILE *fopen_or_die(const char *fname, const char *mode) {
    ...
    if (!f) err(1, "%s (mode '%s')", fname, mode);
    return f;
}

FILE *fopen_chk(const char *fname, const char *mode) {
    FILE *f = fopen(fname, mode);
    if (!f) ythrow yexception() <<  fname << " (mode '" <<  mode << "'): " << LastSystemErrorText();
    return f;
}
