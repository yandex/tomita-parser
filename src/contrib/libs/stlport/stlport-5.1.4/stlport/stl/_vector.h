...
#include <assert.h>
#if defined(_MSC_VER) && !defined(NDEBUG) && !defined(__GCCXML__)
extern "C" {
    __declspec(dllimport) int __stdcall IsDebuggerPresent();
}
#  define __STLASSERT( a ) do { if ( !(a) ) { if (IsDebuggerPresent()) __debugbreak(); else assert(0&&(a)); } } while (0)
#else
#  define __STLASSERT( a ) assert(a)
#endif
...
private:
  ...
  void _M_nofill_insert(size_type __n);
...
public:
  ...
  void yresize(size_type __new_size) {
    if (__new_size < size())
      erase(begin() + __new_size, end());
    else
      _M_nofill_insert(__new_size - size());
  }
...
private:
...
#endif /* _STLP_MEMBER_TEMPLATES */
  {
    ...
    _STLP_TRY {
      _STLP_STD::uninitialized_copy(__first, __last, __result);
      return __result;
    }
