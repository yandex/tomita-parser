...
protected:
  ...
  _Node_base* _M_create_node(const _InputType& __x) {
#else
  ...
  _Node_base* _M_create_node() { //is it used anywhere?
    _Node* __p = this->_M_node.allocate(1);
    _STLP_TRY {
...
#if defined (_STLP_MEMBER_TEMPLATES)
  template <class _InputType>
  iterator _M_insert_templ(iterator __pos, const _InputType& __x) {
#else
  iterator _M_insert_templ(iterator __pos, const_reference __x) {
...
#endif /*!_STLP_DONT_SUP_DFLT_PARAM && !_STLP_NO_ANACHRONISMS*/
    return _M_insert_templ(__pos, __x);
  }
private:
#if defined (_STLP_MEMBER_TEMPLATES)
  template <class _InputIterator>
