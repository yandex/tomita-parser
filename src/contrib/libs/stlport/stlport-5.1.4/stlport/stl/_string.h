...
protected:
  ...
  void _M_terminate_string_aux(const __false_type& is_integral) {
    _STLP_TRY {
      _M_construct_null_aux(this->_M_Finish(), is_integral);
    }
    _STLP_UNWIND(this->_M_destroy_range(0,0))
  }

  void _M_terminate_string_aux(const __true_type& is_integral)
  { _M_construct_null_aux(this->_M_Finish(), is_integral); }
