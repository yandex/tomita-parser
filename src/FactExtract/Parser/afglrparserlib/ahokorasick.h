...
struct COccurrence : public TPair<size_t, size_t>
{
    ...
    COccurrence(size_t _first, size_t _second, size_t ruleNo, bool ambiguous = false, const CInputItem* inputItem = NULL)
        : TPair<size_t, size_t>(_first, _second)
        , m_GrammarRuleNo(ruleNo)
        , m_bAmbiguous(ambiguous)
        , m_pInputItem(inputItem)
    {
    }
...
public:
    ...
    yvector<CTrieRelation>::const_iterator GetChildren(size_t NodeNo) const
    {
...
protected:
    ...
    yvector<int>::iterator GetChildrenAux(size_t NodeNo)
    {
        return m_ChildrenAux.begin() + NodeNo*GetAlphabetSize();
    };

    yvector<int>::const_iterator GetChildrenAux(size_t NodeNo) const
    {
        return m_ChildrenAux.begin() + NodeNo*GetAlphabetSize();
    };
